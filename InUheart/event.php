...
$r = mysql_query("SELECT * FROM `chat_online` WHERE `id`='$id'");
...
if (!empty($_POST['send_content']) && $to)
{
	...
	$sql = "INSERT INTO `chat_event`(`from`,`to`,`time`,`type`,`content`,`source`,`target`) VALUES('$id','$to','$t','msg','$_content','$source','$target')";
...
if($_POST['typing'] == 'yes')
{
	$sql = "INSERT INTO `chat_event`(`from`,`to`,`time`,`type`,`content`,`source`,`target`) VALUES('$id','$to','$t','typing','','$source','$target')";
	mysql_query($sql);
}
