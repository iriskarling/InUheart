...
function send(s)
{
	...
	show_msg(lang.me+replace_em(s),1);
	user.typing = '';
	user.send.push(s);
}
...
function load_event(id)
{
	...
	$.post('event.php',user,function(_data)
	{
		...
		if (_data && _data.events )
		{
			...
			{
				$('.sending').eq(0).html(lang.me+replace_em(_data['send_content'])).removeClass('sending').css('opacity',1);
			}
			...
			for(var i=0;i<_data.events.length;i++)
			{
				...
				{
					show_msg(lang.stranger+replace_em(data.content));
					stop_typing();
				}
...
function show_reconnect()
{
	$('#typing').before($('<div class="noticication"></div>').html(
		'<input type="button" onclick="reconnect()" value="'+lang.reconnect+'" />'
